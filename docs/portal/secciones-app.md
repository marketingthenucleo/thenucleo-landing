...
>
> 1. **Refrescar `ficha-cliente/fichacliente.md`** sección "Pipelines — módulo F1 con SEED hardcoded" — describe el placeholder anterior, no el v3 vivo. Sustituir por la spec del v3 (árbol Pipelines→Campañas→Triggers+Emails, switch Account/PM, drawers, toggle archivados, seed Neus). Referenciar [[ficha-cliente-operativa]] del vault para el modelo operacional.
...
Ver sección "Comunidad pública (cbi) — Tablas nativas" en [[supabase-schema|docs/infra/supabase-schema]].
