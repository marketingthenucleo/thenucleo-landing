...
- `casuisticas_board` — Tablero kanban single-row (`id='global'`) para `work.thenucleo.com/casuisticas/`. Columna `data jsonb` con `{bolsa, newsletter, hibrido, dudas}` + `updated_by` (email). RLS por allowlist `auth.email()` (5 emails admin desde 2026-05-25). Migrada desde localStorage 2026-05-15.
...
Ambas SECURITY DEFINER + allowlist hardcoded en body (5 emails TheNucleo desde 2026-05-25 — añadida `valentina.ramirez@thenucleo.com`) + `RAISE EXCEPTION 'forbidden'` ERRCODE `42501` si email no autorizado. `GRANT EXECUTE TO authenticated`. ⚠️ **Allowlist en 9 sitios ahora** (frontend playbook/fichas-de-producto/ficha-cliente + RLS policies playbook_progreso/playbook_task_feedback/playbook_cliente_servicios + RPC playbook_cliente_detalle + estas 2 RPCs + RPC `catalogos_cliente_get` + Edge Function `bridge_from_portal` con su policy `admins_read_audit` de `bridge_audit_log` desde 2026-05-25). Casuísticas tiene su propia allowlist (3 policies) — al añadir/retirar editor, sincronizar también allí.
