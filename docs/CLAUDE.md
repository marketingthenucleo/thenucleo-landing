...
Ambas SECURITY DEFINER + allowlist hardcoded en body (5 emails TheNucleo desde 2026-05-25 — añadida `valentina.ramirez@thenucleo.com`) + `RAISE EXCEPTION 'forbidden'` ERRCODE `42501` si email no autorizado. `GRANT EXECUTE TO authenticated`. ⚠️ **Allowlist en 7 sitios ahora** (frontend playbook/fichas-de-producto/ficha-cliente + RLS policies playbook_progreso/playbook_task_feedback/playbook_cliente_servicios + RPC playbook_cliente_detalle + estas 2 RPCs). Casuísticas tiene su propia allowlist (3 policies) — al añadir/retirar editor, sincronizar también allí.
