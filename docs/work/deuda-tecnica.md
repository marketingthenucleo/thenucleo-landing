...
- **"Volver atrás" contextual en `/estrategia/` y `/timeline/`** (decisión 2026-05-26, Opción B). Hoy el botón siempre vuelve a `portal/clientes/<id>?sec=general` (Opción C, sin saber de qué sección venía el usuario). Plan futuro: en los 3 workflows Bubble "Open external website" del bridge (botones Estrategia/Timeline/Ficha) añadir parámetro `from_sec = Get sec from page URL`. La URL bridge pasa a `…/estrategia/?id=XYZ&from_sec=tareas`. Work lo lee al cargar, lo persiste en `sessionStorage` (sobrevive el bridge auth + sobrevive Estrategia↔Timeline), y "Volver atrás" usa ese valor para construir `portal/clientes/<id>?sec=<from_sec>`. Si no hay `from_sec` (entrada directa por URL), fallback a `?sec=general`. Fiabilidad 100%. Implementación frontend ~10 min; cambio Bubble en los 3 workflows ~5 min cada uno. Sin esto el usuario pierde el contexto de la sección anterior cada vez que vuelve.
