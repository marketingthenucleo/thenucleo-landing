...
- **Qué:** desconectado el trigger `Notion: Tarea Creada` (`pageAddedToDatabase`); queda solo `Notion: Tarea Actualizada` (`pagedUpdatedInDatabase`), que también captura las altas (Notion pone `last_edited_time = created_time` al nacer). Creado workflow one-shot manual `P4Wpkqo2npPH6Ad5` (**CLEANUP — Dedupe tareas_notion**) para limpiar los 115 duplicados: lectura acotada a `created_date >= 2026-05-25` y **solo borra en Bubble** (el espejo se limpia por SQL aparte). Se eliminó el nodo Supabase-delete que el SDK guardó sin filtro (habría borrado la tabla entera).
...
- **Limpieza EJECUTADA 2026-05-28:** ejec `142562` borró 115/115 en Bubble + 115 en el espejo por SQL → **0 duplicados**, keepers intactos. La credencial Supabase auto-asignada apuntaba a otro proyecto (ejec `142519` falló 404), así que se quitó la lectura Supabase y se hornearon los 115 `bubble_id` en el Code node.
- **Pendiente:** backstop `UNIQUE(notion_id)`; borrar la tarea de prueba `🧪 TEST SYNC pagedUpdated` en Notion; (opcional) archivar el workflow one-shot y borrar el nodo `Tarea Creada` del sync.
...
- **Área:** `ficha-cliente/index.html` + `estrategia/index.html` (clon bit-a-bit).
- **Qué:** dos iteraciones más sobre el F3 PM Board (commits `6b04beb` + `baa75c8`) que dejan el módulo en **estado final** tras el desvío inicial del kanban por fases. El estado final es lo que Ben quería desde el principio.

### Iteración 3 — Board sube al nivel Pipeline + fix dropdown (commit `6b04beb`)
- Antes: el botón "Crear tareas Notion" vivía dentro de cada Campaña. Tenías que entrar 1 por 1 a P1C1, P1C2, P2C1… y crear board por campaña — fragmenta el trabajo del PM, sin sentido.
- Ahora: el botón vive en la vista del Pipeline (P1, P2…). Al pulsar **Crear** agrega tareas de **todas** las campañas activas del pipeline en un solo pool. Cada card lleva una etiqueta `📂 Campaña PxCy` para saber de cuál viene.
- Implementación:
  - Nueva función pura `proposeTasksForPipeline(p)` que itera `p.camps` activas y concatena `tasks` + `warnings` de cada `proposeTasks(c)`.
  - Renombrado `openTasksBoard(pId, cId)` → `openTasksBoardForPipeline(pId)`.
  - `S.tasksBoard.cId` → `pId`. `renderTasksBoard(p, c)` → `renderTasksBoard(p)`.
  - Card PM movida de `renderCampaignView` a `renderPipelineView` (data-action sigue siendo `create-tasks` pero ahora solo `data-pid`, sin `data-cid`).
  - `attachAllHandlers` engancha el board cuando `S.view.kind === 'pipeline'` (antes era `'campaign'`).
  - `buildTasksPayload` incluye `pipeline.campanias[]` con todas las activas (id/code/nombre/kpi/fechas/plantilla_slug).
  - Cada task lleva `campania_code` + `campania_id` para el render y el payload.
- **Fix dropdown que se cerraba al instante:** SortableJS estaba capturando el `mousedown` sobre los `<select>` y arrancando un drag fantasma. Fix: `filter: 'select, option, input, textarea, button, label, [contenteditable="true"]'` + `preventOnFilter: false` en la config de Sortable. Inicialmente añadí también `handle: '.pm-task-header'` para restringir drag a una zona específica, pero en la iteración 4 lo retiré (ver abajo).

### Iteración 4 — Cards compactas + modal Notion al click (commit `baa75c8`)
- Antes (iteración intermedia que NO documenté): la card mostraba INLINE todas las propiedades Notion (estado/fecha/prioridad/área/responsables/aprobador/observadores/estimación/incidencia/comentario). 320px ancho, ~600px alto. Demasiado ruido visual cuando hay 30+ tareas en el pool.
- Ahora: card compacta (220px) que solo muestra lo escaneable de un vistazo, y **click abre modal** centrado con todas las 24 propiedades editables (estilo "Notion page open").
- **Card compacta** — 4 secciones verticales:
  1. Row 1: code-tag · ⚠ warning (si hay) · 🚨 incidencia (si está marcada) · dot circular del estado (color según TASK_ESTADOS — por hacer/en curso/revisión/bloqueada/hecho)
  2. Título (truncado 2 líneas)
  3. Descripción preview (80 chars, truncado 2 líneas, oculto si vacía)
  4. Meta: `📅 23 ene` (fecha corta) · `📂 P1C1` (campaña) · `👤 2` (count responsables)
- **Modal de detalle** (`renderTaskDetailModal`):
  - Overlay backdrop oscuro con blur, modal centrado max-width 600px, animaciones `pm-fade-in` + `pm-slide-up`.
  - Header: code-tag + chip campaña + botón X (cerrar).
  - Título grande contenteditable (h-style 22px).
  - 24 propiedades Notion editables en grid `label : control`:
    - Estado / Fecha entrega / Prioridad / Área / Rol sugerido (selects + date)
    - Responsables (chips multi-valor con dropdown "+ Añadir")
    - Aprobador (single-select)
    - Observadores (chips multi-valor con dropdown "+ Añadir")
    - Estimación (number input + "h" unit, se convierte a min al empujar payload)
    - Incidencia (checkbox con label dinámico "Marcada" / "No")
    - Bloqueado por (read-only, lista códigos PxCy de `depende_de`)
  - Comentario / Descripción (textarea grande, va al body Notion como bloques paragraph)
  - Footer: botón "Listo".
  - Cierre: click backdrop, X arriba derecha, botón Listo, o tecla Esc.
- **Modelo nuevo:** `S.tasksBoard.expandedTaskId: null | taskId` para saber qué tarea tiene su modal abierto. Reset al regenerar/cambiar de pipeline.
- **Drag-vs-tap distinción:** `delay: 120` + `touchStartThreshold: 8` en SortableJS. Click rápido sin mover = abre modal. Mantener pulsado + arrastrar = mueve la card. SortableJS cancela el click si llegó a iniciar drag — el handler `data-action="open-detail"` solo dispara cuando NO hubo drag.
- **Handle dropped:** la iteración 3 usaba `handle: '.pm-task-header'` para drag solo desde una zona. La iteración 4 elimina el handle porque las cards son ahora compactas — drag desde toda la card es más natural cuando hay menos contenido editable inline.

### Constantes Notion canónicas (sin cambios desde iteración inline anterior)
- `TASK_ESTADOS` (5): por hacer / en curso / revisión / bloqueada / hecho. Cada uno con color hex para el dot del card.
- `TASK_PRIORIDADES` (4): baja / media / alta / urgente.
- `TASK_AREAS` (11): estrategia / copy / diseño / meta_ads / crm / newsletter / community / dev / comercial / rrhh / otros — autopoblada desde `rol_sugerido` con `areaForRol(rol)`.
- `TASK_MEMBERS` (8 hardcoded): Ben Sanchis · Alejandro López · Mel Dalmazo · Valentina Ramírez · Camilo Pérez · Damian Ortiz · Joaquín Tagle · Valeria Díez. Fase 0 sin RPC `bub_user`.
- Defaults aplicados por `proposeTasks mk()`: `estado:'todo'`, `fecha_entrega:null`, `prioridad:'media'`, `area_tarea:areaForRol(rol)`, `responsables:[]`, `aprobador:''`, `observadores:[]`, `estimacion_horas:null`, `descripcion:''`, `incidencia:false`.

### Payload final al webhook n8n `eHyXBETcaGSNXqLk` (Fase 1, sigue dry-run)
```json
{
  "version": "1.1",
  "source": "ficha-cliente-pm-board",
  "cliente_bubble_id": "...",
  "pipeline": { "id, code, nombre, objetivo, account", "campanias": [...] },
  "masters": [{ "client_temp_id, titulo" }],
  "tasks": [{
    "client_temp_id, parent_master_id, code, titulo, rol, fase,
     campania_code, campania_id,
     estado, fecha_entrega, prioridad, area_tarea,
     responsables[], aprobador, observadores[],
     estimacion_horas, estimacion_min, descripcion, incidencia,
     depende_de_codes[], origen, metadata"
  }],
  "idempotency_key": "P1-<ts>-<rand>"
}
```

### Estado final del módulo F3 PM Board (commits del día: `d8273af`, `74e4145`, `0b6ecd0`, `6b04beb`, `baa75c8`)
1. ✅ Botón **Crear tareas Notion** en card PM (rol PM, vista Pipeline).
2. ✅ Genera tareas para TODAS las campañas activas del pipeline siguiendo la plantilla canónica del manual PM (5 fases: estrategia → producción mensajes → triggers → creatividades → lanzamiento).
3. ✅ Pool flex-wrap arriba + tareas madre debajo (creadas por el PM con "+ Tarea madre").
4. ✅ Cards compactas con preview esencial. Click → modal Notion con todas las propiedades editables.
5. ✅ Drag-and-drop entre pool y tareas madre (SortableJS connected lists). Distinguido de tap por delay+threshold.
6. ✅ Esc / X / backdrop / botón Listo cierran modal.
7. ✅ Estado efímero in-memory (`S.tasksBoard`). Sin Supabase ni localStorage. Persiste a través del toggle Account↔PM mismo pipeline.
8. ✅ Push dry-run: console.log(payload) + toast. Webhook real pendiente F2 backend Pipelines.

### Pendientes (Fase 1)
- Backend F2 Pipelines en Supabase (tablas `cliente_pipelines/campanias/triggers/emails/whatsapps/creatividades` + RPC `ficha_pipelines_get`). Hasta entonces el board lee del seed hardcoded de Dra. Neuss.
...
- RPC `bub_user` para poblar TASK_MEMBERS dinámicamente (hoy hardcoded 8 nombres).
- `S.proveedorTareas` actualmente fallback a 'notion'. Cuando esté disponible en `ficha_cliente_get` (vía LEFT JOIN bub_agencia), el texto del botón "🚀 Empujar a Notion/ClickUp" se vuelve dinámico.

### Refs commits del día
- `d8273af` — F3 v1: kanban 6 columnas (descartado por feedback Ben).
- `74e4145` — F3 v2: canvas + tareas madre (correcto, sustituye kanban).
- `0b6ecd0` — F3 v2 propagado a `/estrategia/`.
- `6b04beb` — F3 v3: board sube a nivel Pipeline + fix dropdown.
- `7267974` — Ben commit (sync theme) arrastró tarjetas Notion inline (intermedio sin documentar, sustituido por baa75c8).
- `baa75c8` — F3 v4: cards compactas + modal Notion (estado final).

### Vercel deploys
Todos en `READY` excepto el último `baa75c8` que estaba construyendo al cierre de la sesión. Verificable con MCP Vercel o panel.
...
- **Área:** `docs/design-tokens.md` — nueva sección "Recetas reutilizables — patrones canónicos por componente" antes de "Convenciones de actualización".
- **Qué:** consolidación del aprendizaje del rollout en forma reutilizable, pensada como referencia que una IA pueda leer al generar nuevas piezas (portal Bubble o páginas admin work):
  - **Regla de oro:** tabla con la lógica `theme is "no" ↔ -claro` + mnemónico ("is no = -claro siempre"). Error más recurrente del rollout fue la inversión (asociar `is no` con tokens dark).
  - **Patrón A — Repeating Group cell con zebra + hover:** 4 Conditionals inline en orden exacto (zebra dark → zebra light → hover dark → hover light) sobre Visual base heredado del Style. Aplicado a "Group encabezados plantillas" 2026-05-27.
  - **Patrón B — Nav item / Tab con activo + hover + theme:** Style global maneja inactivo (visual base + Conditional theme); inline manejan activo y hover. Aplicado al "Text General" (tabs General/Tareas/Editar) tras cazar bug "tabs blanquitos en light" → corregir `Text_Nav_Item.theme is no` de `texto-terciario-claro` a `texto-secundario-claro`.
  - **Patrón C — Botón / elemento simple con hover:** Visual dark + 2 Conditionals (`theme is no` + `hovered AND theme is no`). Generalizable.
  - **Regla del salto de jerarquía:** lo que en dark es `texto-terciario` (#5C6078) NO mapea simétricamente a `texto-terciario-claro` (#9CA3AF) en light sobre fondo blanco. Tabla con el mapeo correcto: terciario dark → secundario-claro light si va sobre fondo blanco; solo mantener terciario-claro si va sobre fondos coloreados.
  - **Anti-patrones a evitar (6):** inversión de theme, Conditionals duplicadas, repetir `theme is no` cuando ya lo cubre el Style, asumir simetría dark↔light, hex literal en propiedades temables, fusionar Styles a lo bruto.
  - **Bloque específico para work HTML admin:** reglas al añadir CSS con `[data-theme="light"]` overrides, persistencia toggle, meta theme-color, equivalencia tokens Bubble↔Work.
  - **Checklist final para IA** antes de generar pieza nueva (7 preguntas).
- **Por qué:** el rollout dejó conocimiento muy valioso (qué tokens elegir por rol, cómo ordenar Conditionals, regla del salto de jerarquía descubierta empíricamente con el bug `Text_Nav_Item`) que se perdería en commits + scrollback. Necesario tenerlo en sitio único para que cualquier sesión futura (humana o IA) pueda generar piezas nuevas en ambos sitios sin recaer en los mismos bugs.
- **Impacto:** referencia activa para sesiones futuras. Cuando Ben pida "créame X" en work admin o en el portal, la IA debería leer esta sección primero. Reduce riesgo de inversión de theme, mejora contraste de textos en light, blinda orden de Conditionals.
- **Refs:**
  - `docs/design-tokens.md` — sección "Recetas reutilizables — patrones canónicos por componente" + frontmatter actualizado.
  - Bugs origen: `Text_Nav_Item` con jerarquía mal mapeada en light (cazado en tabs General/Tareas/Editar del sidebar work); "Group encabezados plantillas" con 4 Conditionals invertidas + duplicadas (zebra+hover); spinner con `fondo-elevado` cuando `theme is no` (inversión); Style 12px del lote Text con `#FFFFFF` literal.

## 2026-05-27 [PORTAL][FEATURE] — Fase N+1 light theme portal: categoría Text cerrada (sesión tarde)

- **Área:** Bubble Styles categoría `Text` del portal.
- **Qué:** aplicado el patrón canónico (Visual con tokens semánticos + Conditional `Current User's theme is "no"` con variantes `-claro`) a todos los Styles de Text en bloque. Convención por rol: body/heading → `texto-primario`, label/subtítulo → `texto-secundario`, caption/pista/metadata → `texto-terciario`, inverso → `texto-inverso`, link inline → `acento-secundario`, estados (error/aviso/exito/info) → tokens del mismo nombre.
- **Lote pequeño documentado al detalle** (6 Styles 11/12/13px) en `docs/design-tokens.md` sección Text. El resto de Text (headings grandes, body normal, badges) sigue el mismo patrón por rol pero sin inventario nombre-por-nombre — Ben aplicó en bloque al darse cuenta de que era trabajo lineal de cientos de elementos.
- **Bug recurrente cazado:** un Style del lote 12px tenía color `#FFFFFF` literal (no referencia a variable). Reasignado a `texto-primario` antes de pintar el Conditional. Cambio sutil en dark (#FFFFFF → #EDEEF3) pero necesario para que el theme switch funcione (Bubble ignora literals en Conditionals).
- **Decisión Ben — sin renombrado y sin fusión:** los Styles de Text arrastran cientos de elementos cada uno. Renombrar en Bubble es seguro técnicamente (atado por ID interno) pero genera ruido visual; fusionar duplicados (3 Styles funcionalmente idénticos en el lote pequeño tras unificar tamaños) requeriría reasignar elementos uno a uno. **No se toca.** Convivencia aceptada. Anotada deuda menor para limpieza futura con "Find elements in use".
- **Por qué:** mover Text del estado "pendiente — la chicha" del handoff a ✅ cerrada. Adicionalmente **Link saltada** en la misma sesión: Ben confirmó no usar el Style Link en el portal a fecha de cierre, por lo que no requiere Conditional (plantilla canónica conservada en `docs/design-tokens.md` por si entra en el futuro). **Único pendiente para cerrar el rollout Bubble:** los 8 built-in Bubble (sub-fase de cleanup reasignando hex de `Primary/Surface/Background/Destructive/Success/Alert/Text/Primary contrast` a custom equivalentes).
- **Impacto:** todos los elementos de Text del portal ya cambian de color al togglear `Current User's theme`. Visualmente el dark se mantiene idéntico (excepto el sutil #FFFFFF→#EDEEF3 del Style con literal). El light pinta correctamente todos los roles.
- **Refs:**
  - `docs/design-tokens.md` — sección Text actualizada a ✅ con tabla del lote pequeño + convención por rol.
  - `~/.claude/plans/light-theme-portal-handoff-text-builtin.md` — Text marcada ✅.
  - Bubble: categoría `Design → Styles → Text` (sin inventario nombre-por-nombre).
...
- **Qué:** continuación de la Fase 0 cerrada esta misma mañana (campo `User.theme` + RPC v3). Hoy:
  - **23 Color variables custom renombradas a español sin tildes** (suffix coherente con work): `bg-base→fondo-base`, `bg-card→fondo-tarjeta`, `bg-elevated→fondo-elevado`, `bg-hover→fondo-hover`, `bg-active→fondo-activo`, `border-subtle→borde-sutil`, `border-default→borde-medio`, `border-strong→borde-fuerte`, `text-primary→texto-primario`, `text-secondary→texto-secundario`, `text-tertiary→texto-terciario`, `text-inverse→texto-inverso`, `accent-primary→acento-primario`, `accent-primary-hover→acento-primario-hover`, `accent-secondary→acento-secundario`, `accent-secondary-hover→acento-secundario-hover`, `success→exito`, `warning→aviso`, `error→error`, `info→info`, `neutral→neutro`, `violet→violeta`, `pink→rosa`. Cada una con descripción (tipo built-in Bubble). Bubble propagó las refs en todos los Styles automáticamente al renombrar.
  - **23 Color variables claras nuevas creadas** con suffix `-claro` y mismos hex que la columna light de la tabla maestra (`fondo-base-claro=#F5F6F8`, `texto-primario-claro=#111827`, etc.).
  - **Toggle Bubble cableado:** action en el evento del icono moon/sun = `Make changes to Current User → theme = Current User's theme is "no"` (un solo workflow action sin ramas, toggle automático).
  - **Bug intermedio (resuelto en sesión):** entry duplicada `acento-primario-hover` con valor `#2563EB` que debía ser `acento-secundario-hover` (errata copy-paste). Renombrada en sitio sin perder refs.
  - **Limpieza presets Bubble:** borrados 6 Styles huérfanos del kit "Dark theme" pre-instalado (`Filled Dark Destructive`, `Filled Dark Primary`, `Link Dark Destructive`, `Link Dark Primary`, `Outline Dark Destructive`, `Outline Dark Primary`) que nunca se usaron en el portal. Quedaron 5 Buttons reales para pintar.
  - **6 categorías de Styles pintadas** con Visual (refactor hex→variable cuando aplicaba) + Conditional `Current User's theme is "no"` con tokens `-claro`. Resumen por categoría:
    - **Button (5)**: Boton Peligro (`error`/solid, era gradient `error→error` colapsado a solid), Boton Fantasma (transparent + border `borde-medio`), Boton Primario (`acento-primario`), Boton Secundario (`fondo-tarjeta` + border `borde-medio`), Boton Sutil (era "Premium" — refactor a `fondo-elevado` + border `acento-secundario`, identidad sutil azul). Hover combinado `hovered AND theme is "no"` en los 4 que lo necesitan.
    - **Date/Time Picker (3)**: Selector Fecha/Hora, Celda Calendario (5 conditionals: dark/light × hover/selected), Selector Fecha/Hora Sutil.
    - **Dropdown (1)**: Desplegable.
    - **File Uploader (1)**: Subir Archivo (border dashed).
    - **Floating Group (1)**: Grupo Flotante.
    - **Group (10)**: Tarjeta Sutil, Columna Kanban, Encabezado Kanban, Fondo Iconos, Fondo General, Tarjeta KPI Coloreada (asumida azul), Panel, Panel Legacy, Secciones Principales, Tarjeta.
- **Bug crítico recurrente detectado (documentado en design-tokens.md):** algunos Styles tenían `#2A2D3E` y `#252836` como **hex literales** pintados a mano, no como referencia a la variable (`borde-medio` y `fondo-activo` respectivamente). Theme switch no funciona sobre propiedades pintadas como hex literal por mucho Conditional que añadas — Bubble considera el hex "no temable". Detectado primero en `Secciones Principales`. Regla operativa: antes de pintar Conditional en cualquier Style, verificar que las propiedades referencien variable (no hex), reasignar si es hex.
- **Decisiones de naming acumuladas (Styles ES sin tildes, paralelo a Color variables):** Button_Danger→**Boton Peligro**, Button_Ghost→**Boton Fantasma**, Button_Primary→**Boton Primario**, Button_Secondary→**Boton Secundario**, Premium Button→**Boton Sutil** (con identidad azul, refactor visual completo respecto al original que era genérico con border `Gray 70 (DELETED)` huérfano + bg `#1E2D3D` raw + text `#FFFFFF` raw + icon `Primary contrast`). Standard date/time picker→**Selector Fecha/Hora**, date_table_cell→**Celda Calendario**, Premium date/time picker→**Selector Fecha/Hora Sutil**. Standard dropdown→**Desplegable**. Standard file uploader→**Subir Archivo**. Standard floating group→**Grupo Flotante**. Group: Cards Premium→**Tarjeta Sutil**, Col_kanban_tareas→**Columna Kanban**, Fondo_General→**Fondo General**, Group_KPI_Tarjeta_Coloreada→**Tarjeta KPI Coloreada**, Group_Panel→**Panel**, Group_Panel (Legacy)→**Panel Legacy** (revisar "Find elements in use", borrar si 0).
- **Categorías cerradas en la segunda mitad de la sesión (post-Group):** Group Focus (saltada, sin styles), Icon (1: Icono estandar), Input (1: Campo de Texto — refactor completo desde hex raw + Border `Gray 70 DELETED` huérfano), Multi dropdown (1: Desplegable Múltiple), Multi-File Uploader (1: Subir Archivos Múltiples), Multiline Input (1: Área de Texto), Page (1: Página), Picture Uploader (1: Subir Imagen), Popup (1: Modal), Progress Bar (1: Barra de Progreso), Rich Text Input (1: Editor Enriquecido), Search Box (1: Buscador), Shape (saltada, plantilla canónica documentada), Slider Input (saltada, plantilla canónica documentada), Radio Buttons (saltada por Ben), Repeating Group (saltada por Ben). Total final: **17 categorías cerradas o saltadas**.
- **Patrón coherente formularios** (Campo de Texto, Área de Texto, Desplegable Múltiple, Buscador, Editor Enriquecido): todos comparten `fondo-elevado` + `borde-medio` + `texto-primario` + `texto-terciario` para placeholder + focus border `borde-fuerte`. Variantes únicas: Search Box añade icon lupa, Multi dropdown añade chevron, Buscadores/Uploaders varían en border dashed vs solid. **Deuda menor identificada:** Desplegable (single) quedó con `fondo-tarjeta` y Desplegable Múltiple con `fondo-elevado` — decidir si unificar a `fondo-elevado` en próxima pasada.
- **Link y Text cerradas en la sesión tarde del mismo 2026-05-27** (post-pausa "QUIETO PARADO!! Que aqui hay chicha!!"): **Link** confirmada saltada por Ben (sin uso real en el portal — plantilla canónica `Enlace → acento-secundario`/`acento-secundario-claro` conservada en design-tokens.md por si entra en el futuro). **Text** atacada con el patrón canónico aplicado en bloque por rol (heading/body → `texto-primario`, labels → `texto-secundario`, captions → `texto-terciario`, sobre acento → `texto-inverso`, link inline → `acento-secundario`, estados error/aviso/exito/info → variables correspondientes); **sin renombrado ni fusión** porque cada Style arrastra cientos de elementos en uso y el riesgo de ruido era alto. Lote pequeño 11/12/13px (6 Styles) documentado al detalle en design-tokens.md. **1 bug `#FFFFFF` literal cazado** en un Style de Text durante la pasada y reasignado a `texto-primario` (mismo anti-patrón que vimos antes en `Campo de Texto`/`Boton Sutil`/`Secciones Principales`).
- **Únicos pendientes tras esta sesión:** (1) **8 built-in Bubble** (Primary/Surface/Background/Destructive/Success/Alert/Text/Primary contrast) — solo reasignar valor dark via panel Color variables, no admiten Conditional, estrategia recomendada en handoff. (2) **Sincronización portal ↔ work**: RPC `work_set_my_theme(p_theme boolean)` SECURITY DEFINER self-only + cableado JS en los 3 HTML de work (`ficha-cliente`/`estrategia`/`timeline`, líneas `~6009-6020`) para que el toggle escriba a Supabase además de localStorage y al boot lea el `theme` de la RPC. Hoy: portal toggle ya escribe en `bub_user.theme`, work toggle solo en localStorage — el theme no viaja.
- **Handoff actualizado:** `~/.claude/plans/light-theme-portal-handoff-text-builtin.md` con TL;DR refrescado, Link y Text marcadas cerradas, sección 4 ampliada con plan detallado de sync portal↔work (RPC + JS), 2 opciones de prompt para abrir el próximo chat (A = built-in + sync juntos, B = solo sync). Self-contained para retomar sin contexto previo.
- **Saltadas intencionalmente:** Alert (no se usa), CSS Tools/HTML (no son Styles visuales), Image/Map/Video (transparentes al theme), Material Icon (deprecado por Iconify), Checkbox (sin Styles custom según asumimos).
- **Decisión de arquitectura:** Portal Bubble y work mantienen **dos sistemas paralelos** de tokens con nomenclaturas distintas (portal usa `fondo-tarjeta`, work usa `--bg-2`). NO se fuerza equivalencia 1:1; el doc `design-tokens.md` mantiene tabla de equivalencia para que al crear algo en cualquiera de los dos sepamos qué token usar en el otro. Razón: el portal ya tenía 23 custom variables creadas con criterio propio antes de empezar este rollout; reescribirlas a la nomenclatura work duplicaría el trabajo sin ganancia funcional.
- **Impacto:** ningún cambio en producción todavía — los Conditionals viven en los Styles pero solo se activan cuando un user pone `theme = no`. Default Bubble `no` significa que **users nuevos verán light**; users existentes (8 con `theme=NULL` en `bub_user`) siguen viendo dark hasta que toquen el toggle (la condicional `theme is "no"` evalúa a false sobre NULL). Cero rotura visual a usuarios actuales.
- **Doc canónico actualizado:** [`docs/design-tokens.md`](./design-tokens.md) — sección "Inventario de Styles del portal" reemplazada con snapshot real de las 6 categorías cerradas + bug crítico hex literal documentado + tabla maestra con nombres ES finales + estado del rollout granular por categoría.
- **Refs:** Plan completo `~/.claude/plans/si-vamos-a-planificarlo-peppy-karp.md`. Migration Fase 0 (mismo día): `work_current_user_profile_v3_restore_agencia_add_theme` en `cbixhqjsnpuhcrcjppah`. Tabla `bub_user.theme boolean nullable`.

## 2026-05-27 [INFRA][WORK][BUGFIX] — Error Handler captura nodo en payloads `trigger-mode` (panel `/incidencias` deja de mostrar `Desconocido`)

- **Área:** Workflow n8n `HRDQ9Ju4NAIUV0qyhKzlz` (ERRORES — Capturar y Registrar Plataforma), nodo Code "Extraer Datos Error". Panel `work.thenucleo.com/incidencias`. Tabla `n8n_incidencias` (Supabase cbi).
- **Qué:** el normalizador del Error Handler sólo cubría el shape `{ workflow, execution: { error: {...}, lastNodeExecuted } }`. Cuando el fallo ocurre en el propio Trigger node de un workflow (Notion Trigger devuelve 429/502/504 al hacer polling), n8n envía un payload distinto `{ workflow, trigger: { mode: 'trigger'|'internal', error: {...} } }` SIN `execution.*`. El código resolvía `lastNode = exec?.lastNodeExecuted || 'Desconocido'` → todas esas incidencias entraban con `node_name = "Desconocido"`. Resultado visible: 57/59 incidencias históricas del workflow `SYNC CLIENTES — Notion → Bubble` aparecían como "Desconocido" en el panel, sin pista del nodo culpable. Fix: detectar `!raw.execution && raw.trigger?.mode` → setear `node_name = '[Trigger del workflow]'` + `node_type = '[Trigger del workflow]'` + leer `error.message` desde `raw.trigger.error`. Además se concatena `error.description` y `error.httpCode` al `error_message` para no perder contexto en cualquier shape (`"<msg> | <desc> [HTTP 429]"`). Nuevo campo `trigger_mode` en el output del Code para que queries SQL futuras puedan distinguir el patrón (`'trigger'` vs `'internal'` vs `null`).
- **Por qué:** sesión exploratoria triggered por inbox real de `/incidencias` con 32 abiertas. Tras agrupar (22 Gemini rate-limit en `IA Cerebro — Indexar Drive`, 6 SYNC CLIENTES `node_name=Desconocido`, 1 OPS VENTAS timeout) detectamos que el segundo grupo no era debug-able desde el panel — el operador veía "Desconocido" y no podía decidir qué workflow tocar. Causa raíz identificada comparando `raw_payload` de IA Cerebro (con `execution.error.node.name`) vs SYNC CLIENTES (sin `execution`, sólo `trigger.error`). Confirmado en BD: de 59 incidencias históricas de SYNC CLIENTES, 57 tienen shape trigger-mode.
- **Cómo se aplicó:** vía MCP `claude.ai n8n-native` (OAuth completado en sesión). 1) `update_workflow` con SDK code completo (6 nodos preservados, sólo `jsCode` de "Extraer Datos Error" parcheado) → creó draft `c0932173-d1ac-4865-be28-839ca90721e9` SIN tocar el activeVersionId. **Limitación detectada del SDK del MCP:** `update_workflow` desbindea credenciales SIEMPRE (newCredential está pensado para CREAR, no para REFERENCIAR existentes) — la `note` del response avisa "credentials must be configured manually". Producción intacta porque la draft no se publicó. 2) Smoke test lógica con `test_workflow` + pin data en Error Trigger + Claude + Insert (ejec 139828): output `failed_node="[Trigger del workflow]"` + `error_message="The service is receiving too many requests | You have been rate limited"`. Patch validado sin tocar producción. 3) Rebind manual de Ben en UI: doble-click "Claude Analizar Error" → cred Header Auth (ID `LLL40Z5TPEIiWZkM`), doble-click "Insert Supabase Incidencia" → cred Espejo Supabase (ID `13dKSjEd2XZCYpJa`) + publish. 4) Smoke test end-to-end con creds reales (ejec 139842): se insertó fila real en `n8n_incidencias` con `node_name="Trigger del workflow"` (Claude removió los corchetes al pasar por su prompt, lo dejamos así porque queda más limpio), `error_title="HTTP 429 - Too Many Requests en Trigger"`, status `open`. Fila marcada `resolved` inmediatamente (id `6f2aad4e-115b-4e47-b47d-eac6c8fe9e50`).
- **Hallazgo lateral (no requirió cambio):** los 2 Notion Triggers de `SYNC CLIENTES — Notion → Bubble` (`FcTmv78nLjbCb2Ea08qbt`, nodes `Notion - Se agrega un nuevo cliente...` + `Notion - Se agrega modifica un nuevo cliente`) **ya tenían `retryOnFail: true, maxTries: 5, waitBetweenTries: 5000`**. Aplicado en sesión anterior. Razón de que sigan llegando incidencias: 5 × 5s = 25s de cushion total, insuficiente para degradaciones Notion de 1-2 min. **Recomendación abierta (no aplicada):** subir `waitBetweenTries: 5000 → 30000` (30s × 5 = 2.5 min cushion). No se aplica vía MCP porque el update desbindearía las creds Notion+Bubble+Supabase+Clockify del workflow (12 nodos afectados) — riesgo alto. Si se aplica, hacerlo manualmente desde UI sobre los 2 nodos trigger.
- **Lección aprendida (anti-patrón n8n):** `update_workflow` del MCP `claude.ai n8n-native` **siempre desbindea credenciales existentes**, sin importar el nombre que se pase a `newCredential()`. El SDK está diseñado para crear workflows nuevos, no para preservar bindings al actualizar. Workaround para edits quirúrgicos: editar en UI (10-30s) en lugar de pasar por MCP. Si el cambio es estructural y vale la pena el rebind, prevé: 1) reunir IDs de creds antes, 2) hacer el update fuera de horario, 3) verificar rebind inmediato. Nota añadida al final de `docs/infra/n8n-workflows.md` sección "Lecciones aprendidas".
- **Impacto:** futuras incidencias del Error Handler con shape trigger-mode entrarán con `node_name = "Trigger del workflow"` en lugar de `"Desconocido"` → panel `/incidencias` legible al instante. Cero cambio para incidencias execution-mode (ruta original preservada). Cero cambio en topología (6 nodos, mismos IDs lógicos, mismo activeVersionId).
- **Verificación post-deploy:** snapshot BD `n8n_incidencias` tras smoke test ejec 139842 muestra la nueva fila con campos correctos. Total: 196 filas (195 resolved históricas + 0 abiertas tras esta sesión).
- **Refs:** Workflow n8n `HRDQ9Ju4NAIUV0qyhKzlz` (ERRORES — Capturar y Registrar Plataforma), ejecuciones de prueba `139828` (lógica) + `139842` (end-to-end). Doc canónico Error Handler: [`docs/infra/n8n-workflows.md`](../infra/n8n-workflows.md) sección "Error Handler Global". Tabla destino: `public.n8n_incidencias` (Supabase cbi). Panel visor: `work.thenucleo.com/incidencias` ([incidencias.html](../../incidencias.html)).

## 2026-05-27 [WORK][REFACTOR] — F3 PM Board: rediseño UI a canvas + tareas madre (descartado el kanban por fases)

- **Área:** `ficha-cliente/index.html` + `estrategia/index.html` (clon bit-a-bit) + entrada anterior del log queda obsoleta en su descripción UI.
- **Qué:** la primera iteración del F3 PM Board (commit `d8273af`/`0b6ecd0`) usaba 6 columnas kanban por fase (Estrategia / Producción mensajes / Triggers / Creatividades / Lanzamiento / Excluidas). Ben aclara que NO es lo que quería: la idea era un canvas con cards de tarea sueltas que el PM **empaqueta dentro de "tareas madre"** creadas a mano. Reescritura completa de la UI del board manteniendo intacta la lógica de propuesta (`proposeTasks` sin cambios).
- **Modelo nuevo:** `S.tasksBoard = { cId, tasks, masters: [], warnings, dirty, lastPushAt }`. Cada `task.parent_master_id` apunta al master.id si está dentro de una madre, `null` si está suelta en el pool. Los masters los crea el PM via "+ Tarea madre" — son contenedores con título editable (`{ id: 'm-...', titulo }`) que aparecen como cajas grandes con drop zone interna.
- **UI nueva:**
  - **Header** del board: contador `N tareas propuestas · M empaquetadas` + botones `+ Tarea madre`, `↺ Regenerar`, `Cerrar`, `🚀 Empujar a Notion/ClickUp`.
  - **Pool** (sección "Tareas propuestas (sueltas)"): flex-wrap con cards 220px. Todas las tareas generadas aparecen aquí inicialmente.
  - **Lista de masters**: cada master es una caja grande con `⊞ <título editable>` + contador hijas + 🗑 + drop zone interna flex-wrap. Si está vacía muestra "Arrastra aquí las tareas que quieras empaquetar bajo esta madre".
  - **Estado vacío** (sin masters): "No hay tareas madre todavía. Pulsa + Tarea madre para crear un contenedor…".
- **Drag-and-drop**: pool y cada master son `Sortable` con `group: 'pm-board'` (listas conectadas). `onEnd` lee `evt.to.dataset.masterId` (`""` = pool, `m-...` = un master) y actualiza `task.parent_master_id`. Una tarea se puede sacar de una madre arrastrándola al pool o a otra madre. SortableJS sin cambios (misma versión 1.15.2 CDN).
- **Eliminado del approach anterior:**
  - `const TASK_PHASES` (las 6 columnas por fase).
  - `recalcLaunchDeps(board)` (ya no había concepto de excluidas separadas que afectaran al `_lanzar`).
  - Dropdown `↳ Padre` por tarjeta (sustituido por drag visual).
  - Class `.subtask` y prefijo `↳` (la jerarquía ahora se ve visualmente porque la card vive DENTRO de la caja madre).
  - Columna "Excluidas" — la idea de excluir tareas se postpone; si el PM no quiere una tarea, simplemente NO la mete en ninguna madre y al empujar se decide qué hacer con el pool (probablemente: crear tareas en Notion **solo** las que están dentro de un master; las del pool quedan como tareas root sin agrupar).
- **Payload Fase 0 / 1 actualizado:** `buildTasksPayload` añade `masters: [{ client_temp_id, titulo }]` al root del payload + cada task lleva `parent_master_id`. El webhook n8n cuando se cablee tendrá que crear los masters como páginas padre en Notion (1 pass) y luego las tasks con su `parent_master_id` resuelto al `notion_page_id` correspondiente (2 passes igual que antes, pero ahora los padres son los masters explícitos).
- **CSS:** sustituido `.pm-cols/.pm-col/.pm-col-list` por `.pm-pool-wrap/.pm-pool/.pm-pool-label/.pm-masters/.pm-master/.pm-master-head/.pm-master-title/.pm-master-count/.pm-master-delete/.pm-master-list`. Tarjeta `.pm-task-card` ahora tiene `width: 220px` (antes flexible en columna). Media query mobile actualizado: `.pm-task-card { width: 100% }` debajo de 720px.
- **`docs/portal/pm-manual-pipelines.md`** queda **desactualizado** (sigue describiendo 6 columnas). Pendiente reescribir en la próxima sesión cuando el modelo "canvas + masters" se valide con uso real. NO se borra ahora para evitar perder el contexto si revertimos.
- **Por qué:** Ben mostró un sketch claro: "queria como que de las indicaciones del Account se genera un listado de propuesta de tareas reales con su fecha su descripcion su titulo su responsable y ella luego las va empaquetando en tarea madre y visualmente ve como todo". El kanban por fases agrupaba por dimensión técnica (orden firme Estratega→Copy→…) y eso es lo que el PM quiere DECIDIR, no lo que el sistema le impone. La nueva UI le devuelve la decisión: ve todas las tareas planas y compone las agrupaciones que tengan sentido en cada Campaña concreta.
- **Impacto:** ningún cambio en `proposeTasks` (las mismas tareas se generan, con los mismos códigos, dependencias y roles sugeridos). Solo cambia cómo el PM las organiza antes de empujar. El sheet `openTasksSheet` legacy sigue como código muerto. `npm run build` OK.
- **Refs:** commits `0b6ecd0` (primer push F3 kanban) → este commit (rediseño UI). Plan original sigue vigente como base; la decisión de "6 columnas por fase" del plan queda invalidada — el rediseño la sustituye.
...
- **Área:** `ficha-cliente/index.html`, `estrategia/index.html`, `timeline/index.html` (las 3 páginas bridge admin) + nueva RPC Supabase + `docs/infra/supabase-schema.md` + `docs/work/deuda-tecnica.md`.
- **Qué:** las 3 páginas pasan de tener cada una su propio header viejo (brand "TheNucleo · X" + theme toggle + nav-dropdown con links a los work-admin) a compartir un **shell idéntico al del portal Bubble**:
  - **Sidebar fijo izquierda** (245px ≥1024px, drawer móvil con hamburger en <1024px) con logo Núcleo arriba (isotipo SVG 45×45 + logotipo SVG h25) + 6 items (Dashboard / Clientes [activo, pill `#1a1434` + strip lime `#85DB02` 5px × 100%] / Operaciones / Recursos Humanos / Finanzas / Comunidad) → links directos a `portal.thenucleo.com/<seccion>`, Comunidad va a `work.thenucleo.com/comunidad`. Footer: Ajustes / Soporte / Salir. Item "Clientes" link sincronizado con `bubble_id` actual via `syncClientesLink()` → `portal/clientes?resume=<id>`.
  - **Header top 40px** con campo izquierdo vacío (matching portal) + 2 iconos derecha: theme toggle (moon/sun, 36×36 box bg `#12141b` con icono 18×18 stroke 1.25 color `#edeef3`) + avatar 40×40 radius 50% con iniciales y color del `bub_user.color` del logueado.
  - **Bg split:** main page `#0A0A0A`, sidebar `#111111` (lift sutil). Variables light-aware (`--ps-sidebar-bg`, `--ps-icon-bg`, `--ps-active-pill`) para que el toggle theme funcione en ambos temas.
  - **Items menú lateral:** NewBlack 16px weight 500 (Bubble pide 400 pero NewBlack Regular no está en /fonts/ — browser cae a 500, mismo render), color text-secondary `#8B8FA8` inactivo / `#edeef3` blanco en hover y activo.
  - **Iconos sidebar:** Lucide 24×24 viewBox, stroke 1.25 extra-light. Equivalentes Iconify para Bubble: `lucide:layout-grid`, `lucide:users`, `lucide:settings`, `lucide:dollar-sign`, `lucide:message-square`, `lucide:help-circle`, `lucide:log-out`, `lucide:moon`, `lucide:sun`, `lucide:menu`.
- **En `/estrategia/` y `/timeline/` además:** título cliente `.client-name` pasa a 38px/700/#FFFFFF (era 28px/var(--text)), subtítulo `Sector:` a 16px/700/text-tertiary (era 11/600), pill "Activo" a 30×100min radius 8 padding 0/16 (era pill 999px). Sub-pestañas cross-page (General/Tareas/Editar/Estrategia/Timeline) con hover underline 2px × 55% `#2563EB` matching el "Shape HOVER" de Bubble, sin la línea horizontal larga bajo todo el contenedor (esa sobraba). URLs de las tabs corregidas → General/Tareas/Editar van a `portal.thenucleo.com/clientes/<bubble_id>?sec=general|tareas|editar_cliente` (antes Tareas/Editar estaban como `<button disabled>` con toast "próximamente"). Botón "Volver atrás" → `portal/clientes/<id>?sec=general` (Opción C de la decisión 2026-05-26 — Opción B con `from_sec` propagado por Bubble queda como deuda en `deuda-tecnica.md`). Formateo de "Próxima Factura" arreglado: antes mostraba el timestamp ISO en bruto (ej `2026-01-06T23:42:21.766+00:00`), ahora `new Date().toLocaleDateString('es-ES')` → "6 ene 2026" o "Sin factura asociada" si NULL.
- **Tab "Tareas" conditional por agencia (replica workflow Bubble "Only when proveedor_tareas is notion"):** la RPC `work_current_user_profile()` pasa de **v1** (`{bubble_id, nombre, email, color}` de `bub_user`) a **v2** (añade `agencia_id` + `agencia_proveedor_tareas` con LEFT JOIN `bub_agencia`). Frontend en estrategia/timeline guarda el row entero en `window.WORK_USER_PROFILE` + promesa `WORK_USER_PROFILE_READY`. En `renderCliente`, tras fijar los hrefs de los 5 tabs, espera la promesa y hace `tareasTab.style.display = 'none'` si `agencia_proveedor_tareas !== 'notion'`. Graceful fallback: si la RPC falla, tab visible por defecto. Migration: `work_current_user_profile_v2_proveedor_tareas` (DROP + CREATE — RETURNS TABLE cambia, no se puede CREATE OR REPLACE).
- **Limpieza visual heredada del shell viejo:** quitado `body { background-image: radial-gradient(ellipse at top, var(--bg-glow))... }` (verde al 5% que pintaba una mancha sutil arriba del .client-block — herencia del diseño pre-portal-unification, Bubble no tiene nada parecido). Quitado `border-bottom` del `.portal-header` (línea fina extra que sobraba). Título "Estrategia" / "Timeline" / "Ficha de cliente" oculto del header (`.ph-title { display: none }`) para matchear el campo izquierdo vacío del portal.
- **Por qué:** la sensación de cambio entre portal Bubble (donde el usuario admin pulsa "Estrategia" o "Timeline" en un cliente concreto) y work (donde aterriza vía bridge bearer en ~300ms) era demasiado abrupta — colores distintos, header distinto, nav distinto. El objetivo es que el usuario no perciba el salto entre apps. Trabajo iterativo en 17 commits durante la sesión: rollout inicial (3f14626 + 9ac858d + 30d7faf), refinamiento por Property Editor screenshots que Ben fue pasando de Bubble (Shape menu, Group Seccion y IMG, Image A, Image D, Group Filtro Activo, Text Sector, Text Parent group's Client, Shape HOVER, Group Crear notific, Iconify Notificaciones, Group Menu Lateral, Header, Group Subseccion Tareas con su conditional), y respuestas a observaciones del Ben tipo "no hay degradadillo verde en mi portal", "la rayita lime es 5px no 3", "el bg-card es #12141b no #181818", "el pill activo es #1a1434 no rgba(59,130,246,0.12)".
- **Decisiones finales no obvias:**
  - Bell-dot icon OMITIDO en work (en Bubble abre un floating group de notificaciones que aquí no existe).
  - "Volver atrás" siempre va a `?sec=general` (Opción C) en vez de la sección de origen — Opción B requiere tocar 3 workflows Bubble, queda como deuda B.
  - Avatar es un `<div>` con texto + color de fondo dinámico (matching el `Text Iniciales` de Bubble que usa `Current User's color`).
  - RPC v2 NO añade allowlist sitio adicional: filtra por `auth.email()` self-only, sin enumeración.
- **Impacto:** ningún cambio funcional en la lógica de paneles (Pipelines / Catálogos / Servicios / Anomalías / Datos siguen intactos). Solo cambia el "shell" (sidebar + header + título cliente + sub-pestañas). El bridge bearer y el módulo de Pipelines siguen funcionando. RPC v1 → v2 transparente para frontend (campos opcionales nuevos, sin breaking changes; pero por el DROP+CREATE hubo ~5s de ventana donde la RPC no existía — irrelevante en producción work). `npm run build` OK (59 archivos).
- **Refs:** 17 commits hoy desde `30d7faf` hasta `6e38c53`. Migración Supabase: `work_current_user_profile_v2_proveedor_tareas` aplicada en `cbixhqjsnpuhcrcjppah`. Doc canónico schema actualizado: [`docs/infra/supabase-schema.md`](../infra/supabase-schema.md#work_current_user_profile---perfil-del-user-logueado-avatar-shell). Deuda Opción B `from_sec` documentada: [`docs/work/deuda-tecnica.md`](../work/deuda-tecnica.md).
...
## 2026-05-26 [PORTAL][WORK][BUGFIX] — Bridge Portal→Work: bearer mode (Edge Function v6) bypassa bug Toolbox

- **Área:** Edge Function `bridge_from_portal` (v6 ACTIVE) + doc `docs/work/bridge-portal-ficha.md` (revisión mayor). Bubble pendiente de rehacer page workflows.
- **Qué:** la Edge Function ahora soporta **dos modos de autenticación** sobre el mismo `BRIDGE_SHARED_SECRET`. El body con `signature` mantiene el camino HMAC legacy (retrocompat 100%). El body sin `signature` cae a la rama **bearer**: comprueba `Authorization: Bearer <secret>` con comparación timing-safe. `parseTimestamp()` acepta number o numeric string (Bubble nativo expresa unix vía `Current date/time:formatted as X`). Limpieza del debug echo de v5 (filtraba `body_sample` en respuestas 403 — info que un atacante con el secret podría usar para fingerprinting). Allowlist + replay window ±5 min + allowlist `ALLOWED_NEXT_PATHS` siguen idénticos.
- **Por qué:** desbloquear el bridge sin depender del plugin **Bubble Toolbox** (BETA), que en la sesión del 2026-05-26 (entry inmediatamente posterior) demostró ser la causa raíz: `properties.email`/`bubble_id`/`secret` llegan `undefined` al Server Script incluso hardcodeando el hex literal en el value del key. El bug del plugin queda como deuda técnica de baja prioridad (alternativas si se vuelve a necesitar crypto en Bubble: mover lógica a Edge Function/n8n, probar plugins crypto alternativos, o snippet debug que vive archivado en el doc).
- **Trade-off de seguridad:** bearer mode pierde la "firma criptográficamente vinculada al payload" de HMAC, pero como el secret viaja en HTTPS en backend privacy de Bubble en ambos modos y la replay window ±5 min es idéntica, el delta de seguridad real es despreciable para este caso (admin SSO, no flujo público). Documentado en sección "Dos modos de autenticación" + "Seguridad" del bridge doc.
- **Setup Bubble (pendiente de aplicar — manual fuera de Claude):** API Connector `Config - Supabase Bridge` → añadir header `Authorization: Bearer <secret hex literal>` + eliminar `signature` del Body. Page workflow de cada botón pasa de **3 steps** (Server Script + API Connector + Navigate) a **2 steps** (API Connector + Navigate). Receta completa en `bridge-portal-ficha.md` sección 5.1+5.2. Option Set `Config (bridge)` se puede archivar.
- **Impacto:**
  - Edge Function `bridge_from_portal` versión **6 ACTIVE, verify_jwt: false** (id `65fe71ce-88bb-4f62-ac7b-4755377092f2`). Ya sirviendo tráfico.
  - Bubble: API Connector necesita 1 header + 1 quita-campo; cada botón pasa de 3 steps a 2. Smoke-test esperado: 1 row `success=true` por click en `bridge_audit_log`. Botón Ficha (legacy) sin `next_path` para fallback a `/ficha-cliente/?id=<bubble_id>`.
  - **Nueva `failure_reason='bad_bearer'`** posible en `bridge_audit_log` (header faltante, formato inválido, o token != secret).
  - Allowlist `EDITOR_EMAILS` sigue en 9 sitios (sin cambios).
- **Verificación:** curl directo a la Edge Function en ambos modos (bearer + HMAC) debe devolver 200 con `action_link`. Comandos exactos en sección "Verification" del plan + doc Troubleshooting.
- **Refs:** `supabase/functions/bridge_from_portal/index.ts` (reescrito, deployed v6). `docs/work/bridge-portal-ficha.md` (revisión mayor — header con "Dos modos de autenticación" + sección 5 reescrita "Setup Bubble — modo bearer" + sección 5b nueva "Setup Bubble — modo HMAC (legacy)" + diagrama de secuencia bearer + Troubleshooting reordenado por modo + Rotación con bearer header + "Estado actual del rollout" actualizado + bug Server Script marcado como resuelto by-pass arquitectónico). Plan en `~/.claude/plans/wiggly-beaming-lake.md`.
...
- **Iteraciones misma sesión:**
  - **Bug `Error` en checkbox pese a guardar OK** (commit `d8fd260`): los UPSERT funcionaban en DB pero el frontend mostraba toast de error y el checkbox volvía a su estado anterior. Causa: el helper `tableRequest` hacía `res.json()` sobre body vacío que devolvía PostgREST con `Prefer: return=minimal` → `SyntaxError: Unexpected end of JSON input`. Fix doble: (1) helper tolera body vacío (lee como text primero, JSON.parse opcional con try/catch); (2) toggleVisibility cambia `Prefer` a `return=representation` (body siempre llega con la row guardada). Verificado: rows previas de Ben SÍ estaban persistidas en DB con `created_by=benjamin.sanchis@thenucleo.com`.
  - **Cambio semántica a opt-in** (commit pendiente — esta sesión): Ben pide "por defecto todas las macros sin seleccionar, ya las elegirá Valentina". `isHidden()` cambia de `default visible` (sin row → false) a `default oculto` (sin row → true). Cascada nueva: al activar una macro, el frontend envía array UPSERT con la macro + todos sus catálogos en oculto=false (la macro sola serviría de poco si los catálogos siguen ocultos por el default opt-in). Help text del sheet actualizado: "Por defecto todo está oculto. Activa las macros que apliquen…". Empty card actualizado: "Ninguna macro activada todavía…". El esquema DB NO cambia — la columna `oculto` mantiene su significado literal, lo que cambia es solo la INTERPRETACIÓN del caso "sin row".
- **Refs:** migration `f2_7_sprint3_catalogos_visibilidad` aplicada vía MCP. Commits frontend: `7eef03f` (Sprint 3 inicial), `5fa8363` (bugfix botón + Añadir Sprint 2), `d8fd260` (fix checkbox body vacío), pendiente (opt-in). Doc canónico: [[../work/ficha-cliente|docs/work/ficha-cliente]] sección "Visibilidad por cliente" + entrada en "Fixes y cambios recientes". Schema: [[../infra/supabase-schema|docs/infra/supabase-schema]] sección "Catálogos del cliente — F2.7 Fase A" ampliada en esta misma sesión con la tabla de visibilidad.
...
- **INSERT en `comunidad_admins` (mismo día, tarde):** primer test de Valentina disparó `RPC ficha_plantillas_listar HTTP 403 forbidden` — esa RPC y todo el módulo **Pipelines y Campañas** (8 tablas `cliente_pipelines/campanias/triggers/emails/whatsapp/creatividades/plantillas` + sus 30+ policies) usa `is_comunidad_admin()` como gate, no allowlist hardcoded. También `disponibilidades` (3 tablas) y `fichas-de-producto` (2 tablas). `INSERT INTO comunidad_admins (user_id) VALUES ('f2624ac9-e2de-4255-b1f4-d7fc12eabe96')` desbloqueó las 45 policies + 3 RPCs de golpe. **Side-effect aceptado:** Valentina queda también admin de `/comunidad/admin/` (panel moderación pública) — reversible con DELETE si no es el alcance deseado.
...
- **Qué:** Resuelve la queja del piloto de Mel "no sé qué hace cada cosa". Iconos `ⓘ` típicos (tap → bottom sheet con explicación) en los puntos donde el modelo PxCx (Pipeline → Campaña → Trigger → Email) no se entiende a la primera. Acotado al módulo Pipelines y Campañas — el resto de paneles (Datos, Servicios, Catálogos, Anomalías) y las otras páginas admin (playbook, fichas-de-producto, casuísticas, disponibilidades) quedan fuera de scope, por ahora.
- **3 commits secuenciales:**
  - **`c0afa7d` (C1 — infra):** helper `infoIcon(key)` dentro de `PIPELINES_MODULE`. Mapa `INFO_CONTENT` con las 12 entradas de copy completo en castellano, expuesto a `window.FICHA_INFO_CONTENT` para que el handler global lo lea. Overlay secundario apilable sobre `#sheet` (`#info-pop` z-index 120 + `#info-pop-backdrop` z-index 110) — clave para que las ⓘ dentro de drawers abiertos no pisen el form. Handlers globales con delegación (click `[data-info]` abre, click backdrop o ✕ cierra, Esc cierra). CSS `.pip-info-ico` (círculo 18×18 borde `--line-2`, hover/focus `--accent`), `.info-pop` + `.info-pop-backdrop`, h4/p/li/code/strong dentro del pop + `.info-pop-ref` (footer pequeño para citar `.docx`). Smoke por consola: `openFichaInfo('pipeline')`.
  - **`7cd8ef0` (C2 — labels y fields):** 9 inserciones de ⓘ en headers de sección y labels de form. `pipeline` tras el botón "+ Pipeline" en el header del módulo. `campania` en "Campañas dentro de Px" (renderPipelineView). `trigger-tipos` en "Triggers · N" (renderCampaignView) + dentro del drawer Nuevo Trigger en el heading "Tipo de trigger". `triggers-aplicables` en "Emails · N" (renderCampaignView) + en el field-label del email view + en el form-label del email drawer. `objetivo-negocio` en el label del pipeline drawer. `plantilla` y `briefing-drive` en los heading/label del campaign drawer. `trigger-bd-fecha` tras el asterisco rojo de "Fecha de lanzamiento" en el trigger drawer (solo cuando tipo=BD). `versionado` en el `fieldStatic` del email view — para esto se amplía `fieldStatic(label, value, opts)` con `opts.info` opcional que renderiza `infoIcon(opts.info)` tras el label (reutilizable a futuro). La hint legacy "Regla .docx caso 7..." del Versionado se retira porque el contenido vive ahora en `INFO_CONTENT.versionado` con ejemplos completos.
  - **`f4fc460` (C3 — estados y acciones):** 12 inserciones. `estado-fino` en los 4 labels "Estado" de los dropdowns de edición — explica qué marca Account (declarado/archivado) vs qué deriva del lifecycle de tareas Notion (creado, monitorizando, copy-listo, diseño-listo, montado-ghl, en-produccion, activo). `codigo-inmutable` junto a los 4 botones "Editar" — explica regla .docx §3.4 (códigos no se reutilizan). `archivar` junto a los 4 botones "Archivar" — explica regla §9 (no se borra, queda reservado el código, recuperable con "Mostrar archivados"). Cambio estructural: cada par botón+ⓘ se envuelve en nuevo `.pip-action-row` (flex row, gap 8px). Los botones siguen `width:100%` pero como `flex:1` dentro del row, dejando 18px para el icono. Regla CSS legacy `.pip-edit-btn + .pip-archive-btn { margin-top: 12px }` ya no aplica (ahora hermanos de `.pip-action-row`, no del botón directo); sustituida por `.pip-action-row + .pip-action-row { margin-top: 12px }` que preserva el spacing entre las dos filas.
- **Por qué:** Las reglas del modelo (`.docx` §3, §3.4, §4, §5, §7, §9) vivían repartidas entre `docs/portal/ficha-cliente.md` y notas inline pequeñas en la UI. Sin contexto, Mel se perdía con qué es un Pipeline vs Campaña, qué significa cada estado, por qué no puede editar el código, etc. La ⓘ trae la regla al sitio donde se necesita.
- **Decisión UX clave** (Plan agent + sesión): overlay secundario apilable, NO reusar `openSheet()` directo. Si la ⓘ vive dentro de un drawer abierto (ej. "Triggers aplicables" en el drawer Editar email), abrir el info-pop encima del sheet preserva el estado del form. Descartado HTML5 `popover` attribute (quirks iOS Safari + popover-en-popover requiere `popover=manual`), tooltip CSS hover (rompe en touch), reusar `openSheet` (pisaría drawer).
- **Impacto:**
  - ✅ 12 ⓘ visibles en TODOS los clientes (78) cuando se carga la pestaña Pipelines — viven en el JS estático del módulo, no en datos.
  - ✅ A11y: cada ⓘ es `<button type="button">` con `aria-label="Más información"`; el info-pop es `role="dialog" aria-modal="true" aria-labelledby="info-pop-title"`. Tabular con teclado y Enter/Space activan.
  - ✅ Mobile-first: bottom sheet (inset auto 12px 12px) con max-width 520px centrado en desktop, max-height 70vh con overflow-y auto.
  - ❌ Texto del copy es definitivo pero corto — si Mel reporta que algo no se entiende, editar la entrada correspondiente en `INFO_CONTENT` es 1 línea.
- **Refs:**
  - Commits: `c0afa7d` (C1 infra), `7cd8ef0` (C2 labels/fields), `f4fc460` (C3 estados/acciones).
  - Branch: `main`.
  - Plan aprobado: `/root/.claude/plans/puedes-ponerles-una-i-hazy-wombat.md`.
  - 12 keys del mapa: `pipeline`, `campania`, `trigger-tipos`, `trigger-bd-fecha`, `objetivo-negocio`, `triggers-aplicables`, `plantilla`, `briefing-drive`, `estado-fino`, `codigo-inmutable`, `archivar`, `versionado`.
...
- **Qué:**
  - **5 upserts del modelo PxCx:** `ficha_pipeline_upsert`, `ficha_campania_upsert`, `ficha_trigger_upsert`, `ficha_email_upsert` + helper `ficha_archivar_codigo(p_kind text, p_id uuid)` genérico para los 4 tipos. Patrón uniforme: `p_id NULL` = INSERT, `p_id` dado = UPDATE. INSERT auto-asigna código vía regex sobre max actual del contexto (cliente/pipeline/campaña), respetando `p_codigo_override` opcional. UPDATE nunca toca el codigo (regla `.docx` §3.4: codes never reused).
  - **Auto-código por capa:**
    - Pipelines: `P<N>` per cliente (max+1 sobre `cliente_pipelines.codigo` filtrado por `cliente_bubble_id`).
    - Campañas: `<pipelineCodigo>C<N>` per pipeline (max+1 filtrado por `pipeline_id`).
    - Triggers: `<campCodigo><tipo><N>` per (campaña, tipo) — FM1/FM2 son secuencia independiente de FW1, BD1 (max+1 filtrado por `campania_id + tipo`).
    - Emails: sin codigo en DB — sólo `orden int` auto-asignado max+1 por campaña. El código display (`P1C1E2` / `P1C1FM1E2`) se deriva en frontend via `emailCode(camp, email)` en función de `orden + triggers_aplicables`.
  - **Archivar:** `ficha_archivar_codigo('pipeline'|'campania'|'trigger'|'email', uuid)`. Resuelve la diferencia de género en los CHECKs (campañas → 'archivada', resto → 'archivado'). Regla `.docx` §3.6 + caso 9: nada se borra, sólo se archiva.
  - **"Servidor propone, usuario valida":** cada upsert acepta `p_codigo_override text DEFAULT NULL`. Si NULL/'' el servidor autocompone (caso típico). Si pasa valor, lo respeta y el UNIQUE de la tabla valida unicidad. Decisión documentada en la sesión (preferencia de Ben).
  - **Refresh tras save:** todas las RPCs devuelven la row insertada/actualizada como `jsonb` para el frontend, pero el flujo previsto es `await rpc('ficha_X_upsert', {...})` + `await loadFor(bubbleId)` para refrescar el árbol completo desde DB (consistencia garantizada vs merge selectivo — más simple en JS, +100ms por save irrelevante en admin tool).
  - **Smoke test (DO block via execute_sql):** INSERT pipeline auto-código P5 → INSERT campaña dentro auto-código P5C1 → INSERT trigger FM auto-código P5C1FM1 → INSERT trigger BD con `fecha_lanzamiento='2026-06-01'` auto-código P5C1BD1 (CHECK obliga la fecha) → INSERT email orden auto-1 → UPDATE email estado 'declarado' → 'copy-listo' → archivar pipeline → DELETE cleanup. Verde, sin residuos. Neus permanece con sus P1-P4 intactos.
  - **`get_advisors security`:** 0 hits sobre las 5 tablas + 6 RPCs nuevas.
- **Por qué:** Cerrar la mitad write de F2.2 antes de tocar el frontend. Patrón seguro: con backend testeado, el cableo de UI puede iterar rápido sin miedo a regresiones de schema. La separación A (RPCs) / B (frontend) reduce el blast radius si algo falla en una parte.
- **Impacto:**
  - ✅ Backend write 100% disponible. Cualquier cliente Supabase puede crear/editar/archivar pipelines, campañas, triggers, emails desde ya.
  - ✅ Mantiene el principio de "1 sitio de allowlist menos" — todas las RPCs son INVOKER. Sin tocar el contador (sigue en 7).
  - ⚠️ Frontend `/ficha-cliente/` aún no llama a las upserts — los drawers siguen mutando `S.data` en memoria. El banner "modo lectura · F2.2.1" sigue visible y es preciso. Cerrar en F2.2.2.B.
- **Refs:**
  - Commit: pendiente push.
  - Branch: `claude/customer-record-setup-sz960`.
  - Migration: `ficha_cliente_pipelines_f2_write_rpcs`.
  - RPCs nuevas (6): `public.ficha_pipeline_upsert`, `public.ficha_campania_upsert`, `public.ficha_trigger_upsert`, `public.ficha_email_upsert`, `public.ficha_archivar_codigo`.
  - Archivos editados: `supabase/migrations/20260524_ficha_cliente_pipelines_f2_write_rpcs.sql` (nuevo), `docs/infra/supabase-schema.md`, `docs/work/ficha-cliente.md`, este `docs/log-cambios.md`.
- **Siguiente paso (F2.2.2.B):**
  1. Localizar los handlers de save de cada drawer en `ficha-cliente/index.html` (Nuevo Pipeline / Nueva Campaña / Nuevo Trigger / Nuevo Email + edit inline + archivar).
  2. Reemplazar las mutaciones `S.data[i].foo = ...` por `await rpc('ficha_X_upsert', {...})` con try/catch + toast en error.
  3. Tras cada save: `await PIPELINES_MODULE.loadFor(S.bubbleId)` para refresco completo.
  4. Refactor `stateBadge()`: pasar de 3 a 14 estados (2 pipeline + 3 campaña + 4 trigger + 6 email) con labels en castellano + clases CSS por color.
  5. Retirar banner "modo lectura · F2.2.1" en `renderList()` cuando todo esté wired.
