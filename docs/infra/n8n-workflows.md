...
**Trigger:** Notion polling cada minuto. **Desde 2026-05-28 SOLO `pagedUpdatedInDatabase`** — el trigger `created`/`pageAddedToDatabase` (`Notion: Tarea Creada`) se desconectó por la race de duplicados (ver "Fix definitivo duplicados 2026-05-28" abajo). `pagedUpdatedInDatabase` también captura las altas porque Notion pone `last_edited_time = created_time` al nacer la página (verificado con tarea de prueba 2026-05-28).
...
**Fix definitivo duplicados 2026-05-28 (1 solo trigger):** los fixes de 2026-05-13 (lookup Supabase) y 2026-05-18 (dedupe por `notion_id` en `Decidir Acción`) **estrecharon pero NO cerraron** la race. El dedupe solo agrupa items DENTRO de una misma ejecución, pero los dos Notion Triggers disparan **ejecuciones independientes** de n8n: el dedupe no ve la copia de la otra ejecución. Una página recién creada entra a la vez en la ventana de `pageAddedToDatabase` (sort `created_time`) y `pagedUpdatedInDatabase` (sort `last_edited_time`, que al nacer == `created_time`) → 2 ejecuciones paralelas (~3.4 s de diferencia media). Cada una hace su `Buscar en Supabase` antes de que la otra escriba el espejo → ambas deciden `create` → duplicado. **Auditoría 2026-05-28:** 115 `notion_id` duplicados / 230 filas en `bub_tareas_notion`; 112 nacieron con ≤10 s de diferencia, máximo 2 copias. La copia con `last_edited_time` más reciente es la viva; la otra queda congelada (el `limit:1` del lookup resuelve siempre la misma). **Fix aplicado:** desconectar el trigger `Notion: Tarea Creada` (`pageAddedToDatabase`) y dejar solo `Notion: Tarea Actualizada` (`pagedUpdatedInDatabase`), que ya cubre las altas. Verificado con tarea de prueba `36ee4743-b0ae-8123-adf8-cce714522bb7` (1 sola fila, responsable correcto, 0 duplicados). **Importante:** NO es un upsert atómico — la idempotencia real exigiría `UNIQUE(notion_id)` + `ON CONFLICT` como backstop (pendiente; requiere limpiar duplicados antes para no violar la constraint). **Limpieza de los 115 duplicados existentes:** workflow one-shot manual `P4Wpkqo2npPH6Ad5` (**CLEANUP — Dedupe tareas_notion**). Cadena: `Leer espejo (filtra created_date >= 2026-05-25 — la dup más antigua es del 2026-05-25 10:53, así captura ambas copias de los 115 sin escanear las ~860 tareas viejas) → Calcular huérfanos (Code: agrupa por notion_id, conserva la fila de last_edited_time más reciente, emite el resto) → Borrar huérfano en Bubble (onError: continueRegularOutput)`. **Solo borra en Bubble.** La limpieza del espejo `bub_tareas_notion` se hace por **SQL aparte** (`DELETE ... WHERE bubble_id IN (...)`), igual que el fix 2026-05-13, porque `FGxG67I24POOUeHW` no propaga DELETEs. El nodo Supabase-delete original se eliminó: el SDK lo guardó **sin filtro** (habría borrado la tabla entera). La reconciliación `ZqccS38F2Lz8WFwX` NO los arregla (no son huérfanos: existen en Notion). ⚠️ Antes de ejecutar: en la UI asignar las **mismas credenciales que `GjijIDEUyiH05Mg0`** (Supabase service_role → `cbi` + Bubble portal LIVE; el MCP no expone las credenciales para verificarlas), hacer dry-run con el nodo de borrado desactivado y confirmar que `Calcular huérfanos` emite 115, y añadir tag `portal`.
