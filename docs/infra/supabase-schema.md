...
#### `cliente_creatividades` — declaración de piezas por Campaña (NUEVO 2026-05-25, F2.5 + F2.5b)
```
id                uuid PK
campania_id       uuid NOT NULL REFERENCES cliente_campanias(id) ON DELETE CASCADE
...
notas             text
link_brief_drive  text                                  -- F2.5b 2026-05-25. URL del brief específico
                                                         -- de esta declaración (mood board, guion, refs).
                                                         -- Opcional, por-row. Distinto del briefing macro
                                                         -- de Campaña (cliente_campanias.link_briefing_drive).
...
- `ficha_creatividad_upsert(p_id, p_campania_id, p_tipo, p_cantidad, p_notas, p_estado, p_orden, p_link_brief_drive)` — **NUEVO 2026-05-25 (F2.5 + F2.5b)**. CRUD de `cliente_creatividades`. Validate `tipo IN ('estatico','reel','carrusel','copy_RRSS','video','otro')`. Param `p_link_brief_drive` añadido en F2.5b (8 params total).
...
**Frontend cableado (F2.2.2.B, completo desde 2026-05-24):** 4 drawers de `PIPELINES_MODULE` cableados a las 6 RPCs en 5 micro-commits. Cada save hace `await rpc('ficha_X_upsert', {...})` + `await PIPELINES_MODULE.loadFor(bubbleId, { preserveView: true })` (refresh completo + preserva vista actual desde 2026-05-24). `stateBadge()` ampliado a 11 estados finos con CSS agrupado por familia (warn/info/ok/neutral). Banner "modo lectura · F2.2.1" retirado.

**Iteración piloto Mel (2026-05-24, tarde):** archivar trigger/email cableado (botón al final de sus detail views, reusa `ficha_archivar_codigo` con `kind='trigger'|'email'`); edit inline en las 4 detail views (drawers reciben `editing` opcional, prellenado + UPDATE); modelo de email refactorizado para que el código SIEMPRE incluya los subcódigos de triggers (retirada caso 5 "vacío = todos" en write — solo en display fallback para orphans); validación ≥1 trigger marcado en email; campo "Orden" del email drawer retirado (redundante con `E${orden}` del código).

**Catálogo de plantillas — 3 RPCs nuevas (todo cerrado 2026-05-24, tarde):**

- `ficha_plantillas_listar(p_bubble_id text)` RETURNS jsonb. `SECURITY DEFINER` (lee `bub_clientes` para derivar `agencia_id`) + gate `is_comunidad_admin()`. Devuelve plantillas con `estado='activa'` para la agencia del cliente, ordenadas por `(orden, slug)`. Consumida en `loadFor()` del módulo Pipelines junto a `ficha_pipelines_get` (Promise.all). Resuelve la deuda del picker hardcoded en JS const.
- `ficha_plantilla_create_from_campania(p_campania_id uuid)` RETURNS jsonb. `SECURITY DEFINER` + gate `is_comunidad_admin()`. Auto-crea plantilla SHELL en `cliente_campania_plantillas` a partir de una campaña custom recién creada (`!isEdit && state.custom` en el frontend). Captura metadata (nombre, kpi_default, presupuesto_default, briefing_master_url). NO snapshotea `triggers_tipicos` ni `emails_tipicos`. Slug auto (lowercase + translate acentos + collision handling `-N`). Linka la campaña a la nueva plantilla via `UPDATE cliente_campanias.plantilla_id`.
- `ficha_plantilla_archivar(p_id uuid)` RETURNS jsonb. `SECURITY INVOKER` (solo toca `cliente_campania_plantillas` — RLS cubre admin). Soft delete via `UPDATE estado='archivada'`. La plantilla queda en DB pero deja de aparecer en el picker (que filtra activas). Llamada desde el botón × en cada card del picker, tras `confirm()`.
