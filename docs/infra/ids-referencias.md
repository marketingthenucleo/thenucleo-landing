...
| Bubble Data API (nodo nativo) | `i8UMJM5KZOGBRf5z` | bubbleApi nativa — usada por nodos `n8n-nodes-base.bubble` (operaciones CRUD estándar) |
| **Bubble Data API (header HTTP)** | `IFAeIvEVDbrPBZIW` | httpHeaderAuth `Authorization: Bearer 088a20b5...`. **Cred canónica para HTTP Request nodes** con queries Bubble custom (constraints encoded, endpoints no-CRUD). Usada por `8snJvdNsmRM2yI2y` (OPS LOG Pub/Sub), `wvHcgVqqjkWJcJDu` (SYNC Bubble→Notion), `GjijIDEUyiH05Mg0` (SYNC TAREAS Notion→Bubble Activity Log), etc. |
...
| **Gemini API Key (header HTTP)** | `fEKYLWb7Vhx4HnNs` | httpHeaderAuth `x-goog-api-key: AQ.Ab8...`. Cred canónica para HTTP Request nodes que llaman a Gemini API (`generativelanguage.googleapis.com`). Mismo valor que `$env.GEMINI_API_KEY` pero accesible desde HTTP nodes (donde `$env` está bloqueado). Creada 2026-05-24 tras intento fallido de usar `$env` en header. |
...
| `N8N_BLOCK_ENV_ACCESS_IN_NODE` | Controla acceso a `$env.*` desde expressions de nodos | **`true`** (default, no se ha cambiado). **Importante**: este flag bloquea `$env` en `expressions` de nodos (HTTP Request `headerParameters.value`, `url`, etc.) pero **NO afecta a Code nodes** — en Code nodes (jsCode) `$env.*` SIEMPRE funciona. Bajarlo a `false` ampliaría superficie de inyección (cualquier expression de cualquier nodo podría leer env vars). Por eso el patrón canónico es: secrets en Code nodes vía `$env`, secrets en HTTP nodes vía cred Generic Header Auth bindeada. Ver `n8n-workflows.md` lección 20. |
...
| System User | `thenucleoadssync` · ID **`61581615834355`** · rol Admin BM. (⚠️ doc previo decía `122135715861053861` — corregido 2026-05-24 tras verificación visual en Meta BM. El System User real visible en `business.facebook.com/settings/system-users` tiene el ID corto). Existe también un 2º System User `TheNucleo_App_Control_Ads` ID `100091956884423` — propósito histórico, no se usa para los workflows actuales |
| Token Meta | never-expires (en teoría), cifrado en `agencia_integraciones_config` (`addon_slug='meta-ads'`). **⚠️ Puede ser revocado por Meta sin previo aviso** (security review, cambios permisos, etc.) → si workflow `Uqv3R3txzcg8GI1B` empieza a fallar con 403 "Forbidden - perhaps check your credentials?", regenerar token en Meta BM → System User `thenucleoadssync` → "Generar identificador" → app Ads Control Portal → permisos `ads_read`+`ads_management`+`business_management` → cifrar nuevo token vía RPC `aic_set_with_key` |
...
| IA Cerebro — Cargar Contexto Cliente [SUB] | `F1gfvgmQ90JmQlTr` | ✅ Creado 2026-05-24. Encapsula lógica RAG común antes duplicada entre `JI5Tr7IogqXgaI7a` (Preparar Indexacion) y `7yjLwl4cEJa7XAYY` (Process Tools). I/O: `{ conversation_id, cliente_notion_id, save_to_metadata }` → `{ ok, cliente, store_id, resumen, contexto_cargado, error }`. Smart-cache: si `metadata.contexto_cargado=true && contexto_resumen`, devuelve cacheado sin re-llamar Gemini. |
