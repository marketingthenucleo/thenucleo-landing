...
**⚠️ Allowlist:** ya está en **10 sitios** (3 frontends admin + 3 RLS playbook + 4 RPCs `playbook_cliente_detalle`/`ficha_cliente_listar`/`ficha_cliente_get`/`catalogos_cliente_get` + EF `bridge_from_portal` con policy `admins_read_audit` + pareja `work_set_my_theme`/EF `sync_theme_to_bubble` desde 2026-05-27). Threshold 10+ alcanzado — migrar a tabla `admin_emails(email)` + RPC `is_work_admin(email)` o reutilizar `comunidad_admins` antes de añadir el siguiente.
